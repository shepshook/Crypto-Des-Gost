...
namespace DesCrypto
{
    public static class Des
    {
        ...
        {
            var subKeys = GenerateSubKeys(new BitArray(key));
            ...
            for (var blockNumber = 0; blockNumber < messageBlocks.Length; blockNumber++)
            {
                ...
                for (var i = 0; i < 16; i++)
                {
                    ...
                    if (!decrypt)
                        nextRight.Xor(subKeys[i]);
                    else
                        nextRight.Xor(subKeys[^(i + 1)]);
            ...
            {
                var tailLength = 1;
                while (tailLength <= 7 && finalArray[^tailLength] - 1 == finalArray[^(tailLength + 1)]) 
                    tailLength++;
